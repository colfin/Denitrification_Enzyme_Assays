...
## Sediment pH
```{r}
# Define a named vector for matching Dates to Seasons
season_map <- c(
  "20230329" = "Spring (29 March 2023)",
  "20230621" = "Summer (21 June 2023)",
  "20231205" = "Fall (5 December 2023)"
)

# Add a 'Season' column by matching the 'Date' column to the season_map
sed_pH$Season <- factor(season_map[as.character(sed_pH$Date)], levels = c("Spring (29 March 2023)", "Summer (21 June 2023)", "Fall (5 December 2023)"))


# Add combined pool name:
sed_pH$combined_pool_name <- denit_combined_4Seasons$combined_pool_name[9:68]
```

```{r stats assumptions}
# Outliers:
Outliers_Sediment_pH <- sed_pH %>%
  group_by(Season) %>%
  identify_outliers(pH)
Outliers_Sediment_pH
  # There are extreme outliers.
...
sed_pH %>%
  ...
  # QQ plot:
ggqqplot(sed_pH, "pH", facet.by = "Season")
```

```{r Wilcox test: RSC}
# Set factors:
sed_pH$RSC <- as.factor(sed_pH$RSC)

# Wilcox Test, nicer format, can use to verify stats on plot:
pH_wilcox_RSC<- compare_means(pH~RSC, sed_pH, group.by="Season", method = "wilcox.test", paired = F)

# Turn into data frame:
pH_wilcox_RSC <- as.data.frame(pH_wilcox_RSC)
...
```{r Wilcox test: Upstream vs. downstream}
# Set factors:
sed_pH$Up_or_down <- as.factor(sed_pH$Up_or_down)
...
# Wilcox Test, pH in-stream:
pH_wilcox_UpvsDown_IS<- compare_means(pH~Up_or_down, sed_pH_IS, group.by="Season", method = "wilcox.test", paired = T)

# Turn into data frame:
pH_wilcox_UpvsDown_IS <- as.data.frame(NH4_wilcox_UpvsDown_IS)

# Wilcox Test, pH dry:
pH_wilcox_UpvsDown_Dry<- compare_means(pH~Up_or_down, sed_pH_Dry, group.by="Season", method = "wilcox.test", paired = T)

# Turn into data frame:
pH_wilcox_UpvsDown_Dry <- as.data.frame(pH_wilcox_UpvsDown_Dry)
...
```{r Plot sediment pH data}
plot.sed_pH <- ggplot(sed_pH, aes(x=RSC, y=pH)) +
                     geom_boxplot(outlier.shape = NA) +
                     geom_point(aes(shape=combined_pool_name, color=Water_column, fill = Water_column), size = 5, position = position_jitterdodge()) +
  facet_wrap(vars(Season))+
  scale_color_manual(values=c("#E69F00","#009E73","#56B4E9"), labels = c("dry", "island", "submerged"), name="Water Level") +
  ...
  theme(text=element_text(size=20),axis.title=element_text(size=20,face="bold"),
          ...
          panel.border = element_rect(colour = "black",linewidth=1),strip.text = element_text(size = 20), legend.position="none", legend.title = element_text(size=20), legend.text=element_text(size=20)) + 
          theme(axis.ticks.length=unit(0.3,"cm")) + 
          labs(x = "\nRegenerative Stormwater Conveyance (RSC) Type", y = "sediment pH", title="") +
  theme(rect=element_rect(fill="transparent"))+
  theme(plot.background = element_rect(color=NA))+
  stat_compare_means(method = "wilcox.test", paired = F, label = "p.signif", label.y.npc = "center", label.x.npc = "center", size = 10, hide.ns = T)
...
ggsave("../figures/pH_sediment.png", plot=plot.sed_pH, device="png", path=NULL, scale=1, width=12, height=8, dpi=600, limitsize=TRUE, bg="white")
```
