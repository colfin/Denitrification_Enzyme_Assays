...
#setwd("~/GitHub/Denitrification_Enzyme_Assays/HydeCounty/analyses") # for R desktop users
setwd("/cloud/home/r825761/GitHub/Denitrification_Enzyme_Assays/HydeCounty/analyses") # for Posit Cloud users

# Use to set root directory for knitr
#knitr::opts_knit$set(root.dir="~/GitHub/Denitrification_Enzyme_Assays/HydeCounty/analyses") # for R desktop users
```
...
denit_rates <- read.csv("~/GitHub/Denitrification_Enzyme_Assays/HydeCounty/data/denit_rates_HydeCo_20221208.csv")

# Read in denitrification rate calculation with CORRECTED rates for morning samples:
denit_CORRECTED <- read.csv("~/GitHub/Denitrification_Enzyme_Assays/HydeCounty/data/CORRECTED_denit_rates_HydeCo_20221208.csv")
```

```{r plotting}
# Set levels of Salt to make plotting easier:
denit_rates$Salt <- factor(denit_rates$Salt, levels = c("low", "medium", "high", "combined"))

plot.denit_rates <- ggplot(denit_rates, aes(x=Salt, y=Denitrification_rate)) +
                     geom_boxplot(aes(color=Salt), outlier.shape = NA) +
                     geom_point(aes(color=Salt), size = 1.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("gray85","gray44","gray4", "burlywood2"), name="Salinity")+
  theme_bw() +
...
denit_corrected_ACET <- dplyr::filter(denit_CORRECTED, denit_CORRECTED$Acetylene == "ACET", .preserve = TRUE)
```

```{r plot ACET}
plot.denit_ACET <- ggplot(denit_ACET, aes(x=Salt, y=Denitrification_rate)) +
                     geom_boxplot(aes(color=Salt), outlier.shape = NA) +
                     geom_point(aes(color=Salt), size = 1.5, position = position_jitterdodge()) +
...
```{r plot buffer vs field}
plot.buff_v_fied_ACET <- ggplot(denit_ACET, aes(x=Field_or_Buffer, y=Denitrification_rate)) +
                     geom_boxplot(aes(color=Field_or_Buffer), outlier.shape = NA) +
                     geom_point(aes(color=Field_or_Buffer), size = 1.5, position = position_jitterdodge()) +
  scale_color_manual(values=c("brown","forestgreen"), labels = c("buffer", "field"), name="Land Use")+
  theme_bw() +
  #Remove plot grid lines
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  #Set axis title and text properties, tick marks, and labels
